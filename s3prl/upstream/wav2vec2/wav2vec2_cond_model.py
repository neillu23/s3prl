...
from .cond_blocks import CC, DoubleCC, TCAC, DoubleTCAC
# from .ctc import CTC
...
class Wav2Vec2CondModel(nn.Module):
    def __init__(self, cfg: Wav2Vec2Config, cond_cfg: Dict):
        super().__init__()
        self.cfg = cfg
        ...
        self.embed_condition_method = cond_cfg["embed_condition_method"]
        ...
        encoder_cls = TransformerCondEncoder
        if cfg.layer_type == "conformer" and cfg.pos_enc_type in ["rel_pos", "rope"]:
            encoder_cls = ConformerCondEncoder

        self.encoder = encoder_cls(cfg, cond_cfg)
    ...
    def forward(
        ...
        if mask:
            ...
            y = unmasked_features
            mask_indices = None
        x, layer_results = self.encoder(x, condition_features=condition_features, langs=langs, langs_lens=langs_lens, padding_mask=padding_mask, layer=layer, split_forward=split_forward, last_layer_result=last_layer_result, start_layer=start_layer, end_layer=end_layer)
...
class TransformerCondEncoder(nn.Module):
    def build_encoder_layer(self, args: Wav2Vec2Config, cond_cfg: Dict, layer_index: int):
    ...
    def __init__(self, args: Wav2Vec2Config, cond_cfg: Dict):
        super().__init__()
        ...
        self.layers = nn.ModuleList(
            [self.build_encoder_layer(args, cond_cfg, layer_index) for layer_index in range(args.encoder_layers)]
        )
    ...
    def forward(self, x, condition_features=None, langs=None, langs_lens=None, padding_mask=None, layer=None, split_forward=False, last_layer_result=None, start_layer=0, end_layer=24):
        ...
        x, layer_results = self.extract_features(x, condition_features, langs, langs_lens, padding_mask, layer, split_forward=split_forward, last_layer_result=last_layer_result, start_layer=start_layer, end_layer=end_layer)
...
# class ConformerCondEncoder(TransformerCondEncoder):
...
class TransformerSentenceEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        CondLayer = None 
        if cond_cfg["embed_condition_method"] == "CC":
            CondLayer = CC
        elif cond_cfg["embed_condition_method"] == "TCAC":
            CondLayer = TCAC
        elif cond_cfg["embed_condition_method"] == "DoubleCC":
            CondLayer = DoubleCC
        elif cond_cfg["embed_condition_method"] == "DoubleTCAC":
            CondLayer = DoubleAttFiLM3
        else:
        ...
        if layer_index >= cond_cfg["embed_condition_start"] and layer_index < cond_cfg["embed_condition_end"]:
            ...
            self.condition_layer = CondLayer(input_size=embedding_dim, condition_size=cond_cfg["embed_condition_size"], **params)
