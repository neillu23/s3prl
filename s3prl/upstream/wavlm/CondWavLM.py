...
from ..wav2vec2.cond_blocks import CC, DoubleCC, TCAC, DoubleTCAC
...
class CondWavLM(nn.Module):
    def __init__(
        ...
        self.encoder = TransformerCondEncoder(cfg, cond_cfg)
    ...
    def extract_features(
        ...
        # logging.info(condition_features)
...
class TransformerCondEncoder(nn.Module):
    def __init__(self, args, cond_cfg: Dict):
        super().__init__()
    ...
    def forward(self, x, condition_features=None, padding_mask=None, streaming_mask=None, layer=None, split_forward=False, last_layer_result=None, start_layer=0, end_layer=24, pos_bias=None):
        # logging.info(condition_features)
        x, layer_results = self.extract_features(x, condition_features, padding_mask, streaming_mask, layer, split_forward=split_forward, last_layer_result=last_layer_result, start_layer=start_layer, end_layer=end_layer, pos_bias=pos_bias)
    ...
    def extract_features(
        ...
        for i, layer in enumerate(self.layers):
            ...
            if not self.training or (dropout_probability > self.layerdrop):
                # logging.info(condition_features)
                x, (z, pos_bias, padding_mask) = layer(
                    x,
                    condition_features,
...
class TransformerSentenceEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        return x, (attn, pos_bias, self_attn_padding_mask)
